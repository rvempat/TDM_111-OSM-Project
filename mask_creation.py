...
def generate_url(coords):
    ...
    folder_name = "D:/training_dataset/masks"
    if not os.path.exists(folder_name):
        os.makedirs(folder_name)

    for way_id, way_data in coords.items():
        min_lat = way_data["min_lat"]
        min_lon = way_data["min_lon"]
        max_lat = way_data["max_lat"]
        max_lon = way_data["max_lon"]
        map_size = "500,500"
        coordinates = way_data["coordinates"]

        url = f"{base_url}?mapArea={min_lat},{min_lon},{max_lat},{max_lon}"
